...
fetch(`${BACKEND_URI}/api/repos/${store.getState().repository}/${store.getState().branch}/?number=1`)
     .then(res => res.json())
     .then(
         (result) => {
             store.dispatch({
         ...
         (error) => { console.log("Error occurred while retrieving last commit: ", error); }
     );

export function refreshFilesList() {
    // Получение данных для заполнения файлов
    const cwd = store.getState().cwd;
    return fetch(`${BACKEND_URI}/api/repos/${store.getState().repository}?path=${cwd}`)
        .then(res => res.json())
        .then(files_arr => {
            let promises = [];
            for (let i = 0; i < files_arr.length; i++) {
                ...
                promises.push(fetch(`${BACKEND_URI}/api/repos/${store.getState().repository}/blob/${store.getState().branch}/${filepath}?skip_content=true`)
                    .then(res => res.json())
                    .then(fileinfo => {
                            //console.log("Extended file information: ", fileinfo);
                            const sha = fileinfo.last_sha;

                            return fetch(`${BACKEND_URI}/api/repos/${store.getState().repository}/${sha}/?number=1`)
                                .then(res => res.json())
                                .then(
                                    (result) => {
                                        //console.log(result);
            ...
            Promise.allSettled(promises)
                .then((results) =>
                    store.dispatch({
                        ...
                        content: results.map(result => result.value)
                    })
                );
        },
        (error) => {
 ...
 function refreshBranchesList() {
     return fetch(`${BACKEND_URI}/api/repos/${store.getState().repository}/branches`)
         .then(res => res.json())
         .then(
             (result) => {
                 store.dispatch({
