...
function reducer(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case "OPEN_DIRECTORY":
            ...
            let crumbs = new_cwd.split("/").filter((crumb) => crumb !== ".");
            console.log("crumbs_arr: ", crumbs);

            return {...state, cwd: new_cwd, breadcrumbs: crumbs, active_tab: "files"};
        case "SEARCH_FILES":
            new_state = {...state, toFiles: true};
            let files_pattern = action.content;
            new_state.files = state.all_files.filter(object => object.name.includes(files_pattern));
            return new_state;
        case "SEARCH_BRANCHES":
            new_state = {...state, toFiles: true};
            let branches_pattern = action.content;
            new_state.branches = state.all_branches.filter(object => object.name.includes(branches_pattern));
            return new_state;
        ...
        case "BRANCHES_INFO_UPDATE":
            ...
            return {...state, all_branches: action.content, branches: action.content};
        default:
            return state;
    }
}
